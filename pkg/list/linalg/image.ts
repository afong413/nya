...
import { m32Mul } from "./core/util/matrix"
 ...
 */
export function m32TransformImage(
  m: Tys["m32"],
  img: Tys["image2d"],
): Tys["image2d"] {
  ...
  const q1 = m32Mul(m, p1).flat()
  const q2 = m32Mul(m, p2).flat()
  const q3 = m32Mul(m, p3).flat()
...
export default {
  ...
  load() {
    OP_JUXTAPOSE.add(
      ["m32", "image2d"],
      "image2d",
      (m, img) => m32TransformImage(m.value, img.value),
      imageShaderError,
      "",
    )
